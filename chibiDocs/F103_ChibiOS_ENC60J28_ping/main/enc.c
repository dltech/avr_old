...
void enc_reset(struct enc *enc)
{
	...
	// Reset ENC-chip
	chSysLock();
	palClearPad(ENC_RST_PORT, ENC_RST_PIN);
	chThdSleepMicroseconds(100);
	palSetPad(ENC_RST_PORT, ENC_RST_PIN);
	chSysUnlock();
...
uint16_t enc_phy_read(struct enc *enc, uint8_t addr)
{
	uint16_t ret;
	
	enc_register_write(enc, MIREGADR, addr);
	enc_bit_set(enc, MICMD, MICMD_MIIRD);
	
	while(enc_register_read(enc, MISTAT) & MISTAT_BUSY)
		chThdSleepMicroseconds(15);
	
	//enc_bit_clr(enc, MICMD, MICMD_MIIRD);
	enc_register_write(enc, MICMD, 0x00);
	
	ret = enc_register_read(enc, MIRDH) << 8;
	ret = ret | enc_register_read(enc, MIRDL);;
...
uint16_t enc_packet_receive(struct enc *enc, uint16_t maxlen, uint8_t *data)
{
	...
	// Set point of protection zone buffer
	enc_register_write(enc, ERXRDPTL, temp & 0xff);
	enc_register_write(enc, ERXRDPTH, temp >> 8);
...
//	
//	/* Enable interrupts */
//	enc_register_write(enc, EIE, EIE_INTIE | EIE_PKTIE | EIE_TXERIE | EIE_RXERIE);
