...
uint8_t   nretx_eep				EEPROM_SECTION  = 0;
uint8_t   nmodem_eep			EEPROM_SECTION  = 0;
uint8_t   modem_address_eep[48]	EEPROM_SECTION;
uint8_t   retx_address_eep[16]	EEPROM_SECTION;
...
void init (void) {
	...
	if(JUMPER_PIN & JUMPER_RETX) {				// считывает состояния перемычек
		...
		rs485_init();                           // тк это не ретранслятор, инициализируется RS485
		...
		if(JUMPER_PIN & JUMPER_MS) {
			...
			nretx=eeprom_read_byte(&nretx_eep);                                                             // считывает списки модемов и ретрансляторов, привязанных к сети
